...
function createSlidingWindowEffect(TarotEffect) {
	return class SlidingWindowEffect extends TarotEffect {
		...
		applyNearLeft0(slide, percent) {
			...
			const slideWidth =
				this.doorWidth + (this.slideWidth - this.doorWidth) * percent;
		...
		applyNearRight0(slide, percent) {
			...
			const slideWidth =
				this.slideWidth + (this.doorWidth - this.slideWidth) * percent;
		...
		applyNearRight1(slide, percent) {
			...
			// Right-align within slide space
			const offsetX =
				this.paddingLeft + (this.slideWidth - narrowSlideWidth);
