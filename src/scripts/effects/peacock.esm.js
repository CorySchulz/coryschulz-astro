...
function createPeacockEffect(TarotEffect) {
	return class PeacockEffect extends TarotEffect {
		...
		render(frame, utils) {
			...
			// Process each slide
			for (
				let slideIndex = 0, totalSlides = slides.length;
		...
		applyLeftFeather3(slide, percent) {
			...
			const startRotation =
				this.featherRotation + this.featherRotation * this.featherScale; // From feather2
			...
			const rotationAmount = -(
				endRotation +
				(startRotation - endRotation) * percent
			);

			// Progressive scale: featherScale³ at L3 (percent=0) → featherScale² at L2 (percent=1)
			const startScale =
				this.featherScale * this.featherScale * this.featherScale; // Triple compounded at edge
		...
		applyLeftFeather2(slide, percent) {
			...
			const startRotation =
				this.featherRotation + this.featherRotation * this.featherScale; // Max rotation at edge
			const endRotation = this.featherRotation; // Base rotation closer to center
			const rotationAmount = -(
				startRotation +
				(endRotation - startRotation) * percent
			);
		...
		applyLeftFeather1(slide, percent) {
			...
			const scaleAmount =
				this.featherScale + (1 - this.featherScale) * percent;
		...
		applyLeftFeather0(slide, percent) {
			...
			const slideWidth =
				this.featherWidth +
				(this.availableWidth - this.featherWidth) * percent;
		...
		applyRightFeather0(slide, percent) {
			...
			const slideWidth =
				this.availableWidth +
				(this.featherWidth - this.availableWidth) * percent;
		...
		applyRightFeather1(slide, percent) {
			...
			// Right-align within available space
			const offsetX =
				this.leftMargin + (this.availableWidth - featherSlideWidth);
		...
		applyRightFeather2(slide, percent) {
			...
			// Right-align within available space
			const offsetX =
				this.leftMargin + (this.availableWidth - feather2SlideWidth);
			...
			const endRotation =
				this.featherRotation + this.featherRotation * this.featherScale; // Max rotation at edge
			const rotationAmount =
				startRotation + (endRotation - startRotation) * percent;
		...
		applyRightFeather3(slide, percent) {
			...
			// Right-align within available space
			const offsetX =
				this.leftMargin + (this.availableWidth - feather3SlideWidth);
