...
function createStackEffect(TarotEffect) {
	return class StackEffect extends TarotEffect {
		...
		applyStackSpacing() {
			...
			this.ctx.viewport.style.setProperty(
				"--stack-peek-reserve",
				`${reserveSpace}px`,
			);
		}
		...
		applyTopCardPosition(slide, transitionPercent) {
			...
			const finalScale =
				startScale + (endScale - startScale) * transitionPercent;
			...
			const offsetMultiplier =
				...
				(endOffsetMultiplier - startOffsetMultiplier) * transitionPercent;
			const { offsetX, offsetY } = _.calculateStackOffset(
				offsetMultiplier,
				1,
			); // Use full transition for direction calc
		...
		applySecondCardPosition(slide, transitionPercent) {
			...
			const offsetX =
				startOffsetX + (endOffsetX - startOffsetX) * transitionPercent;
		...
		applyThirdCardPosition(slide, transitionPercent) {
			...
			const offsetX =
				startOffsetX +
				(endOffsetX - startOffsetX) * (1 - transitionPercent);
			const offsetY = 0; // No Y offset for horizontal stacking
