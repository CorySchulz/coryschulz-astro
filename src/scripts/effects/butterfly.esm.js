...
*/
//#region src/effects/butterfly.js
	...
	return class ButterflyEffect extends TarotEffect {
		...
		*/
		calculateWidths(frameWidths) {
			...
			const target1 = sideSpace * .6;
			const target2 = sideSpace * .4;
			const target3 = sideSpace * .4;
		...
		*/
		getRotatedWidth(angleDeg, originalWidth, perspective = 1500) {
			...
			return originalWidth * Math.cos(radians) / (1 + originalWidth * Math.sin(radians) / perspective);
		}
		/**
		...
		*/
		applyL1(slide, percent) {
			...
			slide._leftWidthTrimmed = this.slideWidth - this.getRotatedWidth(rotateAmount, this.slideWidth * scaleAmount);
		}
		/**
		...
		*/
		applyR1(slide, percent) {
			...
			slide._scaleTransform = `scale(${scaleAmount})`;
			slide._rotateTransform = `rotateY(-${rotateAmount}deg)`;
			slide._rightWidthTrimmed = this.slideWidth - this.getRotatedWidth(rotateAmount, this.slideWidth * scaleAmount);
		}
		/**
		...
		*/
		render(frame, utils) {
			...
			for (let i = slidesArray.length - 1; i >= 0; i--) {
				...
				slide.style.transform = `translateX(${slide._renderPosition}px) perspective(1500px) ${slide._scaleTransform || ""} ${slide._rotateTransform || ""}`;
			}
		}
		destroy() {
			const _ = this;
			if (_.ctx?.viewport?.style) _.ctx.viewport.style.removeProperty("--tarot-slide-width");
			super.destroy();
		}
	};
}
...
//#endregion
export { createButterflyEffect as default };
