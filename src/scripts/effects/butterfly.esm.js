...
function createButterflyEffect(TarotEffect) {
  return class ButterflyEffect extends TarotEffect {
    ...
    calculateWidths(frameWidths) {
      ...
      _.ctx.viewport.style.setProperty(
        "--tarot-slide-width",
        `${_.slideWidth}px`,
      );
    ...
    getAngleFromFinalWidth(originalWidth, finalWidth, perspective = 1500) {
      ...
      if (finalWidth <= 0 || originalWidth <= 0 || perspective <= 0)
        return null;
    ...
    applyL1(slide, percent) {
      ...
      slide._leftWidthTrimmed =
        this.slideWidth -
        this.getRotatedWidth(rotateAmount, this.slideWidth * scaleAmount);
    }
    ...
    applyR2(slide, percent) {
      const scaleAmount =
        this.L1Scale + (this.L2Scale - this.L1Scale) * percent;
      const rotateAmount =
        this.L1Angle + (this.L2Angle - this.L1Angle) * percent;
      slide.style.transformOrigin = "right center";
      slide._scaleTransform = `scale(${scaleAmount})`;
      slide._rotateTransform = `rotateY(-${rotateAmount}deg)`;
      slide._rightWidthTrimmed =
    ...
    applyR3(slide, percent) {
      const scaleAmount =
        this.L2Scale + (this.L3Scale - this.L2Scale) * percent;
      const rotateAmount =
        this.L2Angle + (this.L3Angle - this.L2Angle) * percent;
